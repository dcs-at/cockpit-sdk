...
var CockpitSDK = function () {
  ...
  _createClass(CockpitSDK, [{
    ...
    key: 'pageGet',
    value: function pageGet(pageName) {
    ...
    key: 'pageData',
    value: function pageData(pageName) {
    ...
    key: 'page',
    value: function page(pageName) {
      var _this3 = this;

      return {
        data: function data(success, error) {
          _this3.pageData(pageName).then(success).catch(error);
        },

        get: function get(success, error) {
          _this3.pageGet(pageName).then(success).catch(error);
        }
      };
    }
  }, {
    ...
    value: function imageSrcSet(assetId) {
      ...
      return widths.map(function (width) {
        if ((typeof width === 'undefined' ? 'undefined' : _typeof(width)) === 'object') return _this4.image(assetId, { width: width.width }) + ' ' + (width.srcSet || width.width ? width.width + 'w' : '');

        return _this4.image(assetId, { width: width }) + ' ' + width + 'w';
      }).join(', ');
    }
  }, {
    key: 'authUser',
